...
class ROIPicker:
    ...
    def __init__(self, img_file, roi_file='', config_file='roi_picker.json'):
        ...
        # TODO: Load `config_file` if exist
    ...
    def run_gui(self, wnd_name='ROI Picker', wait_msec=1):
        ...
        # TODO: Load `self.roi_file` if exist
        ...
        while True:
            ...
            if self.redraw_canvas:
                ...
                self.draw_roi(canvas, self.roi_data[self.roi_selected], self.config_selected)
                self.redraw_canvas = False
    ...
    def process_mouse_events(self, event, x, y, buttons, user_param):
        ...
        if self.roi_selected >= 0:
            if event == cv.EVENT_LBUTTONDOWN:
                # Start to drag a selected point
                self.mouse_lbutton = True
                self.drag_pt_idx = check_on_a_point((x, y), self.roi_data[self.roi_selected]['pts'], self.config['point_dist_threshold'])

            elif event == cv.EVENT_LBUTTONUP:
                # Finish dragging the point
                if self.mouse_lbutton and self.drag_pt_idx >= 0:
                    self.roi_data[self.roi_selected]['pts'][self.drag_pt_idx] = (x, y)
                self.mouse_lbutton = False
                self.drag_pt_idx = -1
                self.redraw_canvas = True

            elif event == cv.EVENT_LBUTTONDBLCLK:
                on_a_point = check_on_a_point((x, y), self.roi_data[self.roi_selected]['pts'], self.config['point_dist_threshold'])
                if on_a_point >= 0:
                    # Remove a selected point
                    self.roi_data[self.roi_selected]['pts'].pop(on_a_point)
                else:
                    if self.roi_data[self.roi_selected]['type'].lower().startswith('points'):
                        # Add a new point if the selected ROI is 'points'
                        self.roi_data[self.roi_selected]['pts'].append((x, y))
                    else:
                        if self.roi_data[self.roi_selected]['type'].lower().startswith('line') and len(self.roi_data[self.roi_selected]['pts']) > 0:
                            on_a_line = check_on_a_line((x, y), self.roi_data[self.roi_selected]['pts'] + [self.roi_data[self.roi_selected]['pts'][0]], self.config['line_dist_threshold'])
                        else:
                            on_a_line = check_on_a_line((x, y), self.roi_data[self.roi_selected]['pts'], self.config['line_dist_threshold'])
                        if on_a_line >= 0:
                            # Insert a new point between a selected line segment
                            self.roi_data[self.roi_selected]['pts'].insert(on_a_line, (x, y))
                        else:
    ...
    def process_key_inputs(self, key):
        ...
        if key == -1: # No key input
            ...
            if len(self.roi_data) == 0:
                ...
                self.roi_selected = (self.roi_selected + 1) % len(self.roi_data)
            self.redraw_canvas = True

        elif key == self.config['key_add_point']:
            self.roi_data.append({'type' : 'points', 'pts'  : [], 'color': randcolor()})
            self.roi_selected = len(self.roi_data) - 1
            self.redraw_canvas = True
            ...
            if self.roi_selected >= 0:
                self.roi_data.pop(self.roi_selected)
            self.roi_selected = len(self.roi_data) - 1
            self.redraw_canvas = True

        elif key == self.config['key_import_roi']:
            self.import_roi_data()

        elif key == self.config['key_export_roi']:
            self.export_roi_data()
            ...
            self.resize_roi_data(self.config['image_scale'] / original_scale)
    ...
    def draw_roi(self, canvas, roi, config):
        ...
        for idx, pt in enumerate(roi['pts']):
            ...
            if config['roi_point_radius'] > 0:
                cv.circle(canvas, center, config['roi_point_radius'], color_inv, config['roi_point_thickness'])
    ...
    def print_status(self, canvas):
        ...
        if self.roi_selected >= 0:
            roi = self.roi_data[self.roi_selected]
            status += f' | {roi["type"]}: {len(roi["pts"])} points'
            cx.putText(canvas, status, self.config['status_offset'], self.config['status_font'], self.config['status_font_scale'], roi['color'], self.config['status_font_thickness'])
        else:
            status += ' | empty ROI'
            cx.putText(canvas, status, self.config['status_offset'], self.config['status_font'], self.config['status_font_scale'], thickness=self.config['status_font_thickness'])


    def resize_roi_data(self, scale):
        '''Resize points in `self.roi_data` with `scale` factor'''
        for roi in self.roi_data:
            for idx, pt in enumerate(roi['pts']):
                roi['pts'][idx] = (scale * pt[0], scale * pt[1])


    def import_roi_data(self):
        return
        data = {}
    ...
    def export_roi_data(self):
        return
        self.resize_roi_data(1 / self.config['image_scale'])
